;(function(window, QB, app, CONFIG, $, Backbone) {
    ...
    $(function() {
        ...
        var ui = {
            ...
            'income_call2': '#income_call2',
            'income_call3': '#income_call3',
            'income_call4': '#income_call4',
        ...
        $(document).on('input', '#search_by_username', function (e) {
            ...
            var isValid = 100 >= userName.length;
            ...
            //     };
            // };
            // console.log('Kondisi pencarian:', app.helpers.renderUsers.condition.filter);
            ...
            if (userName.length >= 1) {
                ...
                app.helpers.renderUsers.condition.filter = filterCondition;
                console.log('Kondisi pencarian:', app.helpers.renderUsers.condition.filter);
        ...
        $(document).on('input', '#search_by_username1', function (e) {
            var searchTerm = e.target.value.trim().toLowerCase(); // Ambil nilai input dan ubah menjadi huruf kecil
            var users = $(".user__name"); // Dapatkan elemen-elemen dengan kelas 'user__name'
        
            var found = false; // Inisialisasi variabel untuk menandai apakah ada data yang ditemukan
        
            users.each(function() {
                var userName = $(this).text().toLowerCase(); // Ambil teks nama pengguna dan ubah menjadi huruf kecil
                var $userItem = $(this).closest(".users__item");
        
                if (userName.includes(searchTerm)) { // Periksa apakah nama pengguna mengandung substring yang dicari
                    $userItem.show(); // Tampilkan elemen user jika cocok dengan pencarian
                    found = true; // Tandai bahwa data ditemukan
                } else {
                    $userItem.hide(); // Sembunyikan elemen user jika tidak cocok dengan pencarian
                }
            });
        
            // Jika tidak ada data yang ditemukan, panggil fungsi scroll dan lakukan pencarian ulang
            if (!found) {
                $(window).scroll(); // Panggil event scroll untuk melakukan pencarian ulang
                $(document).on('input', '#search_by_username');
            }
        });
        ...
        // $(document).on('input', '#search_by_username', function (e) {
        //     var searchTerm = e.target.value.trim().toLowerCase(); // Ambil nilai input dan ubah menjadi huruf kecil
        //     var users = $(".users__item"); // Dapatkan elemen-elemen dengan kelas 'users__item'
        //     var found = false; // Inisialisasi variabel untuk menandai apakah ada data yang ditemukan
        
        //     users.each(function() {
        //         var $userItem = $(this);
        //         var userName = $userItem.find(".user__name").text().toLowerCase(); // Ambil teks nama pengguna dan ubah menjadi huruf kecil
        //         var userEmail = $userItem.find(".user__email").text().toLowerCase(); // Ambil teks email pengguna dan ubah menjadi huruf kecil
        
        //         // Periksa apakah nama pengguna atau email mengandung substring yang dicari
            ...
            app.helpers.join(data).then(function(user) {
                ...
                QB.chat.connect({
                    ...
                    if (error) {
                        ...
                        $(ui.income_call2).modal('show');
                        
                        $(ui.income_call4).modal('hide');
                    }
                });
                ...
                try {
                    ...
                    console.log(error,"ini rtto");
                    
                }
                console.log("error tidak bisa login")
                $('#join_err').modal();
            });
        ...
        $(document).on('click', '.j-user', function() {
            ...
            // Menghapus kelas 'active' dari tombol yang aktif sebelumnya
            $('.j-user.active').removeClass('active');
        
            // Menandai tombol yang diklik sebagai tombol aktif
            $user.addClass('active');
        
            // Menambah atau menghapus pengguna dari objek app.callees
            if ($user.hasClass('active')) {
                // Menetapkan pengguna baru sebagai satu-satunya pengguna yang aktif
                app.callees = { [user.id]: user.name };
                // console.log("ini telephone",app.callees)
                const value = Object.values(app.callees)[0];
                // console.log("iniv",value);
            } else {
                // Menghapus pengguna dari objek app.callees jika tombol diklik lagi
                delete app.callees[user.id];
            }
        });
        ...
        $(document).on('click', '.j-actions', function() {
            ...
            if ($btn.hasClass('hangup')) {
                ...
                // Set codec, bitrate, and fallback options based on connection type
                var mediaParams123 = {
                    video: {
                        bitrate: bandwidth ? parseInt(bandwidth) : 256, // Limit video bitrate
                        width: { max: 640 },  // Lower video resolution
                        height: { max: 480 },
                        frameRate: { max: 15 } // Reduce frame rate for stability
                    },
                    audio: {
                        bitrate: 64,  // Low audio bitrate for stability
                        codec: 'opus'
                    },
                    options: {
                        videoCodec: 'VP8',
                ...
                // Create a new session with adjusted bandwidth settings
                ...
                // Get user media and handle call
                    ...
                    if (err || !stream.getAudioTracks().length || (isAudio ? false : !stream.getVideoTracks().length)) {
                        ...
                        app.currentSession.call({}, function() {
                            if (!window.navigator.onLine) {
                                ...
                                // Retry call after 5 seconds if connection fails
                                setTimeout(() => {
                                    if (window.navigator.onLine) {
                                        app.currentSession.call({}, function() {
        ...
        // Monitor network status changes and show alerts for offline/online
        window.addEventListener('offline', function() {
            app.helpers.stateBoard.update({'title': 'no_internet', 'isError': 'qb-error'});
        });
        
        window.addEventListener('online', function() {
            app.helpers.stateBoard.update({'title': 'reconnected'});
        });
        ...
        QB.webrtc.onSessionCloseListener = function onSessionCloseListener(session){
            ...
            if(document.querySelector('.j-actions[hidden]')){
                console.log("ini satud");
                document.querySelector('.j-actions[hidden]').removeAttribute('hidden');
            }
            if(document.querySelector('.j-caller__ctrl')){
                $(ui.income_call2).modal('show');
                // console.log("ini userId",userId,"ini")
                pressPgDown()
                // pressPgDown().then(() => {
                //     return renameLatestFile(app.caller.full_name);
                // })
            // var newName = prompt("Enter new name for the latest file:");
            function formatCallees(callees) {
                var formattedString = '';
                for (var key in callees) {
                    if (callees.hasOwnProperty(key)) {
                        formattedString += key + '-' + callees[key] + '\n';
                    }
                }
                return formattedString.trim();
            }
            // var newName = formatCallees(app.callees);
            const value = Object.values(app.callees)[0];
            var newName = value +"-"+ localStorage.getItem("AcountId");
console.log("newName",newName)
                if (newName) {
                    pressPgDown(); // Call pressPgDown function first
            
                    // Define a recursive function to retry renaming after a delay
                    function retryRename() {
                        callPythonFunction("rename_latest_file", "POST", { new_name: newName })
                            ...
                            .catch(error => {
                                ...
                                setTimeout(retryRename, 3000);
                            });
                    }
            
                    // Call the retryRename function to start the process
                    setTimeout(retryRename, 3000);
                }
                // renameLatestFile(app.caller.full_name);
                console.log("ini session",app.callees,"ini")
                    // renameLatestFile(userId)
...
///
function callPythonFunction1(action) {
  ...
  function pressPgUp1() {
    callPythonFunction1("press_pgup");
  }

  // Fungsi untuk menekan tombol PgDown
  function pressPgDown1() {
    callPythonFunction1("press_pgdown");
  }

  function callPythonFunction(action, method, data = null) {
    var xhr = new XMLHttpRequest();
    xhr.open(method, "http://127.0.0.1:5000/" + action, true);
...
// Function to press the PgUp key
function pressPgUp() {
    callPythonFunction("press_pgup", "GET");
}

// Function to press the PgDown key
function pressPgDown() {
    callPythonFunction("press_pgdown", "GET");
}

// Function to rename the latest file
function renameLatestFile(newName) {
    const data = { new_name: newName };
    callPythonFunction("rename_latest_file", "POST", data);
}
///
        ...
        QB.webrtc.onRejectCallListener = function onRejectCallListener(session, userId, extension) {
            ...
            console.log('UserId 3 cek: ' + userId);
            console.log('Session: ' + session);
            console.log('Session: ' + session.opponentsIDs);
